...
const createPoint = (i, r) => {
	...
	/* Center the spiral */
	const xPosition = canvas.width / 2 // for make animation in middle x
	const yPosition = canvas.height / 2 // for make animation in middle y
...
const drawCircle = (tick, point) => {
	/* Hue */
	const h = tick * 4 // color rotate speed
...
const draw = () => {
	window.requestAnimationFrame(draw) // animtion loop
	
	const { width, height } = canvas // destructing object
